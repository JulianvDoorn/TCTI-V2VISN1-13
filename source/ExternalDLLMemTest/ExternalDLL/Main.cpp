...
bool studentEdgeDetection = false;
bool studentThresholding = false;
...
int main(int argc, char * argv[]) {
	...
	if (strcmp(implementation, "default") == 0) {
		std::cout << "Implementation: default" << std::endl;
		studentEdgeDetection = false;
	}
	else if (strcmp(implementation, "student") == 0) {
		std::cout << "Implementation: student" << std::endl;
		studentEdgeDetection = true;
	}
	else {
		std::cout << "No such implementation: " << implementation << std::endl;
		exit(-1);
	}
	...
	if (!ImageIO::loadImage(image, *input)) {
...
bool executeSteps(DLLExecution * executor) {
	...
	if (!executor->executePreProcessingStep3(studentEdgeDetection)) {
	...
	if (!executor->executePreProcessingStep4(studentThresholding)) {
